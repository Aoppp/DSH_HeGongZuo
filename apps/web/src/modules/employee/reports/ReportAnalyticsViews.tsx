...
export function DashboardView({ date, revision }: { readonly date: string; readonly revision: number }) {
  ...
  return <>
    <div className="report-dashboard__cards">{cards.map(([label, value, employees]) => <button key={label} onClick={() => label === '当日应提交' ? setExpectedOpen(true) : setSelected({ title: `${label}人员`, employees })}><span className="report-dashboard__card-label">{label}{label === '当日应提交' && <i title="部分人员固定不参与日报统计" aria-label="查看不参与日报统计的人员"><Info size={13} /></i>}</span><strong>{value}</strong><small>{label === '当日应提交' ? '查看名单与人数说明' : '点击查看人员'}</small></button>)}</div>
