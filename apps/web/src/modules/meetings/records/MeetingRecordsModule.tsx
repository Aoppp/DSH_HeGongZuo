import { CalendarDays, Download, FileText, LoaderCircle, Search, Users, X } from 'lucide-react'
...
import { exportMeetingRecordToWord } from './meeting-word-export'
...
export function MeetingRecordsModule(_props: ModuleProps) {
  ...
  const [exporting, setExporting] = useState(false); const [exportError, setExportError] = useState<string | null>(null)
  ...
  function closeDetail() { setDetail(null); setExportError(null); if (window.location.search) window.history.replaceState(null, '', '/meetings') }
  async function exportDetail() { if (!detail || exporting) return; setExporting(true); setExportError(null); try { await exportMeetingRecordToWord(detail, detailView) } catch { setExportError('导出失败，请稍后重试。') } finally { setExporting(false) } }
  ...
  return <div className="meeting-records module-page"><header className="meeting-records__heading"><div><h1>会议管理</h1><p>查看会议摘要与原始记录</p></div><span>{total} 场会议</span></header>
    ...
    <section className="meeting-records__list">{loading ? <SkeletonList count={6} /> : records.length === 0 ? <div className="meeting-records__empty">暂无符合条件的会议记录</div> : records.map((record) => <article key={record.id}><div className="meeting-records__identity"><strong><span>{record.id}</span>{record.title}</strong><p><time>{time(record.startedAt)}</time><em>{record.mode === 'chinese' ? '中文' : '双语'}</em><span className="meeting-records__participants"><Users size={14} />{record.participants.length ? record.participants.map((item) => item.name).join('、') : '未记录参会人员'}</span></p></div><div className="meeting-records__actions"><button type="button" disabled={!record.hasSummary} onClick={() => void openRecord(record.id, 'summary')}><FileText size={15} />{record.hasSummary ? '查看摘要' : '无会议摘要'}</button><button type="button" onClick={() => void openRecord(record.id, 'transcript')}><FileText size={15} />查看原文</button></div></article>)}</section>
    {pages > 1 && <nav className="meeting-records__pagination"><span>第 {page} / {pages} 页</span><Pagination page={page} totalPages={pages} onChange={setPage} label="会议分页" /></nav>}
    {(detail || detailLoading) && <div className="meeting-detail" role="dialog" aria-modal="true"><button className="meeting-detail__backdrop" type="button" aria-label="关闭" onClick={closeDetail} /><section><header><div><small>{detailView === 'summary' ? '会议摘要' : '会议原文'}</small><strong>{detail?.id}　{detail?.title}</strong>{detail && <span>{time(detail.startedAt)}—{time(detail.endedAt)}</span>}</div><div className="meeting-detail__header-actions"><button className="meeting-detail__export" type="button" disabled={!detail || detailLoading || exporting} onClick={() => void exportDetail()}>{exporting ? <LoaderCircle className="spin" size={16} /> : <Download size={16} />}{exporting ? '正在导出…' : '导出 Word'}</button><button className="meeting-detail__close" type="button" aria-label="关闭" onClick={closeDetail}><X size={19} /></button></div></header><main>{exportError && <div className="meeting-detail__export-error">{exportError}</div>}{detailLoading ? <SkeletonDetail /> : detail && <MeetingMarkdown text={detailView === 'summary' ? detail.summary ?? '' : detail.transcript} />}</main></section></div>}
  </div>
}
