...
export type EmployeeExportScope = 'employed' | 'departed'

// 与在职 Excel 列序一致，年龄、工龄和合同剩余天数为动态计算值。
const employedExportColumns: readonly ExportColumn[] = [
...
// 与离职 Excel 列序一致；离职人员不包含在职专用的试用、转正和合同字段。
const departedExportColumns: readonly ExportColumn[] = [
  { header: '序号', value: (_employee, index) => index + 1 },
  { header: '所属公司', value: (employee) => employee.companyName ?? '' },
  { header: '姓名', value: (employee) => employee.displayName },
  { header: '入职时间', value: (employee) => employee.hireDate },
  { header: '离职日期', value: (employee) => employee.departureDate ?? '' },
  { header: '一级部门', value: (employee) => employee.departmentName },
  { header: '二级部门', value: (employee) => employee.departmentLevel2 ?? '' },
  { header: '职位', value: (employee) => employee.jobTitle },
  { header: '用工类型', value: (employee) => employmentTypeZh[employee.employmentType] },
  ...
  { header: '出生日期', value: (employee) => employee.birthDate ?? '' },
  { header: '年龄', value: (employee) => employeeAge(employee.birthDate) ?? '' },
  { header: '工龄（月）', value: (employee) => tenureMonths(employee.hireDate, employee.departureDate) ?? '' },
  { header: '联系方式', value: (employee) => employee.workPhone },
  { header: '邮箱', value: (employee) => employee.personalEmail ?? '' },
  { header: '公司邮箱', value: (employee) => employee.workEmail ?? '' },
  { header: '学历', value: (employee) => employee.education ?? '' },
  { header: '专业', value: (employee) => employee.major ?? '' },
  { header: '毕业学校', value: (employee) => employee.school ?? '' },
  { header: '毕业时间', value: (employee) => employee.graduationDate ?? '' },
  ...
  { header: '居住住址', value: (employee) => employee.residentialAddress ?? '' },
  { header: '身份证地址', value: (employee) => employee.idAddress ?? '' },
  { header: '银行卡号', value: (employee) => employee.bankAccount ?? '' },
  { header: '支行信息', value: (employee) => employee.bankName ?? '' },
  { header: '离职原因（公司内部查看使用）', value: (employee) => employee.departureReason ?? '' },
  { header: '档案编号', value: (employee) => employee.archiveNo ?? '' },
  { header: '备注', value: (employee) => employee.notes ?? '' },
]
...
function exportDetails(scope: EmployeeExportScope): { columns: readonly ExportColumn[]; title: string; fileLabel: string } {
  return scope === 'departed'
    ? { columns: departedExportColumns, title: '王叔和离职人员', fileLabel: '王叔和离职人员' }
    : { columns: employedExportColumns, title: '王叔和在职员工', fileLabel: '王叔和在职员工' }
}

export async function exportEmployeesToExcel(employees: readonly EmployeeRecord[], scope: EmployeeExportScope): Promise<void> {
  const ExcelJS = await import('exceljs')
  const workbook = new ExcelJS.Workbook()
  const details = exportDetails(scope)
  const sheet = workbook.addWorksheet(details.title)
  sheet.columns = details.columns.map((column) => ({
    header: column.header,
    key: column.header,
    width: column.header === '备注' || column.header.includes('住址') ? 36 : 16,
  }))
  employees.forEach((employee, index) => {
    sheet.addRow(details.columns.map((column) => column.value(employee, index)))
  })
  ...
  link.href = url
  link.download = `${details.fileLabel}-${todayText()}.xlsx`
  link.click()
  URL.revokeObjectURL(url)
}

// PDF 导出：打开只读打印窗口，在系统打印对话框中选择“另存为 PDF”
export function exportEmployeesToPdf(employees: readonly EmployeeRecord[], scope: EmployeeExportScope): void {
  const printWindow = window.open('', '_blank', 'width=1400,height=900')
  if (!printWindow) return
  const details = exportDetails(scope)
  const headerCells = details.columns.map((column) => `<th>${column.header}</th>`).join('')
  const rows = employees.map((employee, index) => {
    const cells = details.columns.map((column) => `<td>${String(column.value(employee, index))}</td>`).join('')
    return `<tr>${cells}</tr>`
  }).join('')
