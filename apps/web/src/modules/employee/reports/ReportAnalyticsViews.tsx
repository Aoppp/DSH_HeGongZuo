...
export function DashboardView({ date, revision }: { readonly date: string; readonly revision: number }) {
  const [data, setData] = useState<SubmissionDashboard | null>(null), [trend, setTrend] = useState<readonly SubmissionTrendDay[] | null>(null), [error, setError] = useState(''), [selected, setSelected] = useState<{ title: string; employees: readonly SubmissionEmployee[] } | null>(null), [expectedOpen, setExpectedOpen] = useState(false)
  ...
  const trendMax = Math.max(...(trend ?? []).map((item) => item.expected), 1)
  return <><div className="report-dashboard__cards">{cards.map(([label, value, employees]) => <button key={label} onClick={() => label === '当日应提交' ? setExpectedOpen(true) : setSelected({ title: `${label}人员`, employees })}><span>{label}</span><strong>{value}</strong><small>{label === '当日应提交' ? '查看名单与人数说明' : '点击查看人员'}</small></button>)}</div><div className="report-dashboard__actions"><button onClick={() => void exportMissing(data)}><Download size={15} />导出未提交名单</button><button onClick={() => void exportDelayed(data)}><Download size={15} />导出延后提交名单</button></div><section className="report-dashboard__charts"><article><header><span>当日提交完成率</span><strong>{rate}%</strong></header><div className="report-dashboard__progress"><i style={{ width: `${rate}%` }} /></div><dl><div><dt>已提交</dt><dd>{data.submitted}</dd></div><div><dt>未提交</dt><dd>{data.missing}</dd></div><div><dt>延后提交</dt><dd>{data.delayed}</dd></div></dl></article><article><header><span>近 14 天提交趋势</span><small>以 {date} 为截止日</small></header><div className="report-dashboard__trend">{trend?.map((item) => <div key={item.date} title={`${item.date}：已提交 ${item.submitted}/${item.expected}，未提交 ${item.missing}，延后 ${item.delayed}`}><i style={{ height: `${Math.max(4, item.submitted / trendMax * 100)}%` }} /><em style={{ height: `${Math.max(0, item.missing / trendMax * 100)}%` }} /><small>{item.date.slice(5)}</small></div>)}</div><footer><span>绿色：已提交</span><span>红色：未提交</span></footer></article></section><section className="report-card"><header><h2>部门完成情况</h2><span>{data.date}</span></header><div className="report-departments">{data.departments.map((department) => { const departmentRate = department.expected ? Math.round(department.submitted / department.expected * 100) : 0; return <button key={department.name} onClick={() => setSelected({ title: `${department.name}提交情况`, employees: data.employees.filter((item) => item.department === department.name) })}><div><strong>{department.name}</strong><span>{department.submitted}/{department.expected} 人</span></div><i><b style={{ width: `${departmentRate}%` }} /></i><small>完成 {departmentRate}%・未提交 {department.missing}・延后 {department.delayed}</small></button> })}</div></section>{selected && <EmployeeDialog {...selected} onClose={() => setSelected(null)} />}{expectedOpen && <ExpectedEmployeesDialog employees={data.employees} excluded={data.excluded} onClose={() => setExpectedOpen(false)} />}</>
}
