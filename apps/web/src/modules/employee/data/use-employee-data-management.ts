...
export function useEmployeeDataManagement() {
  ...
  useEffect(() => { if (!successMessage) return; const timer = window.setTimeout(() => setSuccessMessage(null), 2600); return () => window.clearTimeout(timer) }, [successMessage])
  ...
  const applyRecognized = (values: EmployeePasteValues) => { setDraft((current) => current ? { ...current, ...values } : current); setFormError(null) }
