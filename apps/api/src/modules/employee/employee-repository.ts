...
export class PostgresEmployeeRepository {
  ...
  async listPage(options: { readonly query: string; readonly scope: 'employed' | 'departed'; readonly page: number; readonly pageSize: number; readonly sort: 'hireDate' | 'departureDate' | 'tenure' | 'displayName' | 'departmentName' | 'contractEndDate'; readonly ascending: boolean }): Promise<{ readonly employees: EmployeeRecord[]; readonly total: number }> {
    const sortColumns = { hireDate: 'hire_date', departureDate: 'departure_date', tenure: '(departure_date - hire_date)', displayName: 'display_name', departmentName: 'department_name', contractEndDate: 'contract_end_date' } as const
    const filters = [options.scope === 'departed' ? "status = 'inactive'" : "status <> 'inactive'"]
    const values: unknown[] = []
    const query = options.query.trim()
    if (query) { values.push(`%${query}%`); filters.push('(display_name ILIKE $1 OR work_email ILIKE $1 OR personal_email ILIKE $1 OR department_name ILIKE $1 OR department_level2 ILIKE $1 OR job_title ILIKE $1 OR work_location ILIKE $1 OR company_name ILIKE $1)') }
    const where = `WHERE ${filters.join(' AND ')}`
    const count = await this.pool.query<{ total: string }>(`SELECT count(*)::text AS total FROM employees ${where}`, values)
    const offset = (Math.max(1, options.page) - 1) * Math.min(100, Math.max(1, options.pageSize))
    values.push(Math.min(100, Math.max(1, options.pageSize)), offset)
    const result = await this.pool.query<EmployeeRow>(`SELECT ${columns} FROM employees ${where} ORDER BY ${sortColumns[options.sort]} ${options.ascending ? 'ASC' : 'DESC'} NULLS LAST, id ASC LIMIT $${values.length - 1} OFFSET $${values.length}`, values)
    return { employees: result.rows.map(toEmployee), total: Number(count.rows[0]?.total ?? 0) }
  }

  async listForExport(options: { readonly query: string; readonly scope: 'employed' | 'departed'; readonly sort: 'hireDate' | 'departureDate' | 'tenure' | 'displayName' | 'departmentName' | 'contractEndDate'; readonly ascending: boolean }): Promise<EmployeeRecord[]> {
    const sortColumns = { hireDate: 'hire_date', departureDate: 'departure_date', tenure: '(departure_date - hire_date)', displayName: 'display_name', departmentName: 'department_name', contractEndDate: 'contract_end_date' } as const
    const filters = [options.scope === 'departed' ? "status = 'inactive'" : "status <> 'inactive'"]
    const values: unknown[] = []
