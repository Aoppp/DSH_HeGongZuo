...
export function useEmployeeDataManagement() {
  ...
  const loadEmployees = useCallback(async () => { setLoading(true); setDataError(null); try { const result = await readEmployeeRecords({ query, scope: employeeScope, page: currentPage, pageSize: employeesPerPage, sort: sortField, ascending: sortAscending }); setEmployees(result.employees); setTotalEmployees(result.total) } catch (error) { setDataError(error instanceof Error ? error.message : String(error)) } finally { setLoading(false) } }, [currentPage, employeeScope, employeesPerPage, query, sortAscending, sortField])
  ...
  const saveEmployee = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); if (!draft) return; if (editorMode === 'departure') { const date = draft.departureDate?.trim() ?? ''; const reason = draft.departureReason?.trim() ?? ''; if (!date || !reason) { setFormError('请填写离职日期和离职原因。'); return }; setSaving(true); setFormError(null); try { const saved = await departEmployeeRecord(draft.id, date, reason); setEmployees((current) => current.map((employee) => employee.id === saved.id ? saved : employee)); setEmployeeScope('departed'); closeEditor() } catch (error) { setFormError(error instanceof Error ? error.message : String(error)) } finally { setSaving(false) }; return }; const error = validateEmployee(draft, employees); if (error) { setFormError(error); return }; setSaving(true); setFormError(null); try { const resume = pendingResume ? await resumeUploadPayload(pendingResume) : resumeRemoved ? null : undefined; const saved = editorMode === 'create' ? await createEmployeeRecord(normalizeEmployee(draft), resume) : await updateEmployeeRecord(normalizeEmployee(draft), resume); setEmployees((current) => editorMode === 'create' ? [...current, saved] : current.map((employee) => employee.id === saved.id ? saved : employee)); closeEditor() } catch (saveError) { setFormError(saveError instanceof Error ? saveError.message : String(saveError)) } finally { setSaving(false) } }
  return { employees, totalEmployees, loading, dataError, setDataError, loadEmployees, query, setQuery, employeeScope, setEmployeeScope, sortField, setSortField, sortAscending, setSortAscending, currentPage, setCurrentPage, employeesPerPage, setEmployeesPerPage, totalPages, pageEmployees: employees, tableWrapRef, editorMode, draft, formError, saving, pendingResume, resumeInputKey, openEditor, openDepartureEditor, closeEditor, updateDraft, selectResume, removeResume, saveEmployee }
}
