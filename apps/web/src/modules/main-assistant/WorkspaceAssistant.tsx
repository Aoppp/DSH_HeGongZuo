  ...
  return <div className="work-assistant module-page">
    ...
    <div className="work-assistant__layout">
      ...
      <section className="work-assistant__conversation panel-card">
        <header><div><h2>任务处理</h2><p>分析结果会直接回复；如需生成文件，请明确说明文件类型和内容。</p></div><button className="work-assistant__clear" type="button" onClick={() => void clearConversation()} disabled={!session.workspace || session.busy || clearing}>{clearing ? <LoaderCircle className="spin" size={15} /> : <RotateCcw size={15} />}清空对话</button></header>
