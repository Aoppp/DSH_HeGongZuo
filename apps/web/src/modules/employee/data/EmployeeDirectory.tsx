...
import { exportEmployeesToExcel } from './export-employees'
import { employmentTypeLabels, pageSizeOptions, sortFieldsFor, type EmployeeScope, type PageMode } from './employee-data-types'
...
export function EmployeeDirectory(props: EmployeeDirectoryProps) {
  ...
  return <section className="employee-data__panel">
    ...
    <div className="employee-data__toolbar">
      ...
      {props.pageMode === 'directory' && <div className="employee-data__export"><button className="employee-data__secondary" type="button" onClick={() => void exportFile()}><Download size={15} /> 导出 Excel</button></div>}
    ...
    <div className="employee-data__table-wrap" ref={props.tableRef}><table className={`employee-data__table${departed ? ' employee-data__table--departed' : ''}`}><thead><tr>{departed ? <><th>员工</th><th>部门 / 岗位</th><th>联系方式</th><th>入职时间</th><th>离职时间 / 工龄</th><th>离职原因</th><th>{props.pageMode === 'directory' ? '档案' : '操作'}</th></> : <><th>员工</th><th>部门 / 岗位</th><th>联系方式</th><th>用工类型</th><th>入职时间</th><th>合同到期</th><th>{props.pageMode === 'directory' ? '档案' : '操作'}</th></>}</tr></thead><tbody>{!props.loading && visible.map((employee) => <tr key={employee.id}>{departed ? <><td><strong>{employee.displayName}</strong><small>{employee.id}</small></td><td><strong>{employee.departmentName}</strong><small>{employee.jobTitle}</small></td><td><strong>{employee.workPhone}</strong><small>{employee.workEmail ?? '未填写邮箱'}</small></td><td>{employee.hireDate}</td><td><strong>{employee.departureDate ?? '—'}</strong><small>工龄 {tenure(employee)}</small></td><td>{employee.departureReason ?? '—'}</td></> : <><td><strong>{employee.displayName}</strong><small>{employee.id}</small></td><td><strong>{employee.departmentName}</strong><small>{employee.jobTitle}</small></td><td><strong>{employee.workPhone}</strong><small>{employee.workEmail ?? '未填写邮箱'}</small></td><td><span className={`employee-status employee-employment-type--${employee.employmentType}`}>{employmentTypeLabels[employee.employmentType]}</span></td><td>{employee.hireDate}</td><td>{employee.contractEndDate ? <><strong>{employee.contractEndDate}</strong><small>{(() => { const days = contractDaysLeft(employee.contractEndDate); return days === null ? '' : days < 0 ? `已到期 ${-days} 天` : `剩余 ${days} 天` })()}</small></> : '—'}</td></>}<td><div className="employee-data__row-actions">{props.pageMode === 'directory' ? <button type="button" title="查看员工档案" onClick={() => props.onView(employee)}><Eye size={14} /></button> : <><button type="button" title="编辑" onClick={() => props.onEdit(employee)}><Pencil size={14} /></button>{employee.status !== 'inactive' && <button className="is-danger employee-data__departure" type="button" onClick={() => props.onDeparture(employee)}>员工离职</button>}</>}</div></td></tr>)}</tbody></table>{props.loading ? <SkeletonTable columns={7} rows={Math.min(props.employeesPerPage, 8)} header={false} /> : visible.length === 0 && <div className="employee-data__empty">没有找到匹配的员工</div>}</div>
    {!props.loading && visible.length > 0 && <nav className="employee-data__pagination" aria-label="员工分页"><div className="employee-data__pagination-summary"><span>第 {props.currentPage} / {props.totalPages} 页</span><label>每页<select value={props.employeesPerPage} onChange={(event) => props.onEmployeesPerPageChange(Number(event.target.value) as (typeof pageSizeOptions)[number])}>{pageSizeOptions.map((size) => <option key={size} value={size}>{size}</option>)}</select>人</label></div><Pagination page={props.currentPage} totalPages={props.totalPages} onChange={props.onPageChange} label="员工分页" /></nav>}
  </section>
}
