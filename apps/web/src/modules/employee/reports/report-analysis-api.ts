...
export interface ReportAnalysisResult { readonly id?: string; readonly content: string; readonly reportCount: number; readonly references: readonly ReportAnalysisReference[]; readonly generatedAt?: string }
export interface ReportAnalysisVersion extends ReportAnalysisResult { readonly id: string; readonly generatedAt: string }
...
export async function analyzeReports(input: { readonly startDate: string; readonly endDate: string; readonly question?: string }): Promise<ReportAnalysisResult> {
  ...
  return { content: body.content, reportCount: body.reportCount, references: body.references.filter((reference): reference is ReportAnalysisReference => Boolean(reference && typeof reference.id === 'string' && typeof reference.name === 'string' && typeof reference.date === 'string')), ...(typeof body.id === 'string' ? { id: body.id } : {}), ...(typeof body.generatedAt === 'string' ? { generatedAt: body.generatedAt } : {}) }
}

export async function readReportAnalysisVersions(startDate: string, endDate: string): Promise<readonly ReportAnalysisVersion[]> {
  const response = await fetch(`/api/daily-reports/analysis/versions?${new URLSearchParams({ startDate, endDate })}`, { credentials: 'same-origin' })
  ...
  return body.versions.filter((item): item is ReportAnalysisVersion => Boolean(item && typeof item.id === 'string' && typeof item.generatedAt === 'string' && typeof item.content === 'string' && typeof item.reportCount === 'number' && Array.isArray(item.references)))
}
