...
export function EmployeeDataManagement({ backLabel = '返回员工查询', onBack }: EmployeeDataManagementProps) {
  ...
  const employedCount = management.employees.filter((employee) => employee.status !== 'inactive').length
  const departedCount = management.employees.length - employedCount
  const pageTitle = pageMode === 'organization' ? '组织架构图' : pageMode === 'maintenance' ? '员工数据维护' : departed ? '离职人员档案' : '在职员工信息'
  const pageDescription = pageMode === 'organization' ? '按一级部门和二级部门查看在职员工组织分布。' : pageMode === 'maintenance' ? '在系统中查询、新增、查看、编辑或办理员工离职。' : departed ? `当前共 ${departedCount} 名离职人员，按离职时间排序。` : `当前共 ${employedCount} 名在职员工，查看员工档案与组织信息。`
