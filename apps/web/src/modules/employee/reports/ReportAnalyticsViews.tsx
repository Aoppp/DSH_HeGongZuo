...
export function EmployeeArchiveView({ revision }: { readonly revision: number }) {
  ...
  return <><section className="report-card"><header><div><h2>员工日报档案</h2><span>{scope === 'active' ? '在职员工的全部历史日报' : '离职员工的历史日报归档'}</span></div><div className="report-profile-tools"><div className="report-profile-scope"><button className={scope === 'active' ? 'is-active' : ''} onClick={() => setScope('active')}>在职员工</button><button className={scope === 'departed' ? 'is-active' : ''} onClick={() => setScope('departed')}>离职归档</button></div><label className="report-profile-search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="按姓名搜索" /></label></div></header>{visibleProfiles.length ? <div className="report-profile-grid">{visibleProfiles.map((profile) => <button key={profile.id} onClick={() => void open(profile)}><div><strong>{profile.name}</strong><span>{profile.department}{profile.departmentLevel2 ? ` / ${profile.departmentLevel2}` : ''}</span></div><dl><div><dt>提交天数</dt><dd>{profile.submittedDays} 天</dd></div><div><dt>延后天数</dt><dd>{profile.delayedDays} 天</dd></div><div><dt>未交天数</dt><dd>{profile.missingDays} 天</dd></div></dl></button>)}</div> : <div className="daily-reports__empty">没有找到匹配的员工</div>}</section>{selected && <div className="report-dialog" role="dialog" aria-modal="true"><button className="report-dialog__backdrop" onClick={() => setSelected(null)} aria-label="关闭" /><section className="report-dialog__wide"><header><div><strong>{selected.name}的日报档案</strong><span>全部历史日报</span></div><button onClick={() => setSelected(null)} aria-label="关闭"><X size={18} /></button></header><main><h3>历史日报</h3>{historyLoading ? <Loading /> : <div className="report-history">{history.map((report) => <article key={report.record_id}><strong>{report.report_date}</strong><p>{report.today_summary || '未填写工作总结'}</p><span>{new Date(report.submit_time).toLocaleString('zh-CN')}</span></article>)}</div>}</main></section></div>}</>
}
