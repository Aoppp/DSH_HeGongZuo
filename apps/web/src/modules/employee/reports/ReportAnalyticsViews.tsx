...
export function EmployeeArchiveView({ startDate, endDate, revision }: { readonly startDate: string; readonly endDate: string; readonly revision: number }) {
  ...
  useEffect(() => { setProfiles(null); setError(''); void readEmployeeReportProfiles(startDate, endDate).then(setProfiles).catch((reason) => setError(reason instanceof Error ? reason.message : '员工日报档案读取失败。')) }, [startDate, endDate, revision])
