...
function ExpectedEmployeesDialog({ employees, excluded, onClose }: { readonly employees: readonly SubmissionEmployee[]; readonly excluded: readonly { readonly name: string; readonly reason: '请假' | '未排班' | '单独汇报' }[]; readonly onClose: () => void }) {
  const reasons = excluded.filter((item) => item.reason === '请假' || item.reason === '未排班')
  return <div className="report-dialog" role="dialog" aria-modal="true"><button className="report-dialog__backdrop" onClick={onClose} aria-label="关闭" /><section><header><strong>当日应提交人员</strong><button onClick={onClose} aria-label="关闭"><X size={18} /></button></header><div className="report-dialog__list">{reasons.length > 0 && <><p>不纳入当日应提交统计</p>{reasons.map((item) => <article key={`${item.reason}-${item.name}`}><div><strong>{item.name}</strong><span>{item.reason}</span></div></article>)}</>}<p>当日应提交名单</p>{employees.map((employee) => <article key={employee.id}><div><strong>{employee.name}</strong><span>{employee.department}{employee.departmentLevel2 ? ` / ${employee.departmentLevel2}` : ''}</span></div></article>)}</div></section></div>
}
...
export function DashboardView({ date, revision }: { readonly date: string; readonly revision: number }) {
  ...
  return <><div className="report-dashboard__cards">{cards.map(([label, value, employees]) => <button key={label} onClick={() => label === '当日应提交' ? setExpectedOpen(true) : setSelected({ title: `${label}人员`, employees })}><span>{label}</span><strong>{value}</strong><small>点击查看人员</small></button>)}</div><div className="report-dashboard__actions"><button onClick={() => void exportMissing(data)}><Download size={15} />导出未提交名单</button><button onClick={() => void exportDelayed(data)}><Download size={15} />导出延后提交名单</button></div><section className="report-card"><header><h2>部门完成情况</h2><span>{data.date}</span></header><div className="report-departments">{data.departments.map((department) => { const rate = department.expected ? Math.round(department.submitted / department.expected * 100) : 0; return <button key={department.name} onClick={() => setSelected({ title: `${department.name}提交情况`, employees: data.employees.filter((item) => item.department === department.name) })}><div><strong>{department.name}</strong><span>{department.submitted}/{department.expected} 人</span></div><i><b style={{ width: `${rate}%` }} /></i><small>完成 {rate}%・未提交 {department.missing}・延后 {department.delayed}</small></button> })}</div></section>{selected && <EmployeeDialog {...selected} onClose={() => setSelected(null)} />}{expectedOpen && <ExpectedEmployeesDialog employees={data.employees} excluded={data.excluded} onClose={() => setExpectedOpen(false)} />}</>
}
