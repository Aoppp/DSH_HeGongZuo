...
const configPath = process.env.HEGONGZUO_AGENT_RUNTIME_CONFIG ?? '.runtime/account-agent-runtimes.json'
const definitions = JSON.parse(await readFile(configPath, 'utf8'))
if (!Array.isArray(definitions)) throw new Error('账号运行时配置无效。')
...
const checks = await Promise.all(runtimes.map(async ({ port }) => {
  ...
  if (response.status >= 500) throw new Error(`账号运行时 ${port} 不可用。`)
}))
void checks
