...
export async function readEmployeeRecords(options: { readonly query: string; readonly scope: 'employed' | 'departed'; readonly page: number; readonly pageSize: number; readonly sort: string; readonly ascending: boolean }): Promise<EmployeeListResponse> {
  const parameters = new URLSearchParams({ query: options.query, scope: options.scope, page: String(options.page), pageSize: String(options.pageSize), sort: options.sort, ascending: String(options.ascending) })
  return apiRequest<EmployeeListResponse>(`/api/employees?${parameters}`)
}
