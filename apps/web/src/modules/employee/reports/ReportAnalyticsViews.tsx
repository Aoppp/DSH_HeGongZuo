...
import {
  readEmployeeReportProfiles, readIndividualReporters, readReportQuality, readSubmissionDashboard, readSubmissionTrend,
  type EmployeeReportProfile, type EmployeeReportProfileScope, type IndividualReporter, type QualityFinding, type SubmissionDashboard, type SubmissionEmployee, type SubmissionTrendDay,
} from './report-analytics-api'

export type AnalyticsView = 'dashboard' | 'employees' | 'individual' | 'quality'
...
function ExpectedEmployeesDialog({ employees, excluded, onClose }: { readonly employees: readonly SubmissionEmployee[]; readonly excluded: SubmissionDashboard['excluded']; readonly onClose: () => void }) {
  const leave = excluded.filter((item) => item.reason === '请假')
  const unscheduled = excluded.filter((item) => item.reason === '未排班')
  const statisticsExcluded = excluded.filter((item) => item.reason === '不参与统计')
  ...
  return <div className="report-dialog" role="dialog" aria-modal="true"><button className="report-dialog__backdrop" onClick={onClose} aria-label="关闭" /><section className="report-expected-dialog"><header><div><strong>当日应提交说明</strong><span>应提交人数已排除请假、未排班及不参与统计人员</span></div><button onClick={onClose} aria-label="关闭"><X size={18} /></button></header><main className="report-expected"><div className="report-expected__summary"><article><span>应提交</span><strong>{employees.length}</strong><small>人</small></article><article><span>请假</span><strong>{leave.length}</strong><small>人</small></article><article><span>未排班</span><strong>{unscheduled.length}</strong><small>人</small></article></div><section><header><div><h3>不参与日报统计</h3><span>固定不纳入应提交、未提交及趋势数据</span></div><strong>{statisticsExcluded.length} 人</strong></header><div className="report-expected__excluded">{statisticsExcluded.map((item) => <span key={`statistics-${item.name}`}><strong>{item.name}</strong><em>不统计</em></span>)}</div></section><section><header><div><h3>请假与未排班人员</h3><span>不纳入当日应提交统计</span></div><strong>{leave.length + unscheduled.length} 人</strong></header>{leave.length + unscheduled.length === 0 ? <p className="report-expected__empty">当日没有请假或未排班人员</p> : <div className="report-expected__absence">{leave.map((item) => <article key={`leave-${item.name}`}><header><strong>{item.name}</strong><em>请假</em></header>{item.leaveDetails.length ? item.leaveDetails.map((detail, index) => <div key={`${detail.startTime}-${index}`}><span>{detail.type || '请假'}・{formatTime(detail.startTime)} 至 {formatTime(detail.endTime)}</span><p>{detail.reason || '未填写请假理由'}</p></div>) : <div><p>暂无审批详情</p></div>}</article>)}{unscheduled.map((item) => <article key={`unscheduled-${item.name}`}><header><strong>{item.name}</strong><em>未排班</em></header></article>)}</div>}</section><section><header><div><h3>当日应提交名单</h3><span>以下人员需提交日报</span></div><strong>{employees.length} 人</strong></header>{employees.length === 0 ? <p className="report-expected__empty">当日没有应提交人员</p> : <div className="report-expected__employees">{employees.map((employee) => <article key={employee.id}><strong>{employee.name}</strong><span>{employee.department}{employee.departmentLevel2 ? ` / ${employee.departmentLevel2}` : ''}</span></article>)}</div>}</section></main></section></div>
}

export function DashboardView({ date, revision }: { readonly date: string; readonly revision: number }) {
  const [data, setData] = useState<SubmissionDashboard | null>(null), [trend, setTrend] = useState<readonly SubmissionTrendDay[] | null>(null), [error, setError] = useState(''), [selected, setSelected] = useState<{ title: string; employees: readonly SubmissionEmployee[] } | null>(null), [expectedOpen, setExpectedOpen] = useState(false), [activeTrendDate, setActiveTrendDate] = useState<string | null>(null)
  useEffect(() => { const controller = new AbortController(); setData(null); setTrend(null); setError(''); void Promise.all([readSubmissionDashboard(date), readSubmissionTrend(date)]).then(([dashboard, history]) => { if (!controller.signal.aborted) { setData(dashboard); setTrend(history) } }).catch((reason) => { if (!controller.signal.aborted) setError(reason instanceof Error ? reason.message : '看板读取失败。') }); return () => controller.abort() }, [date, revision])
  ...
  const rate = data.expected ? Math.round(data.submitted / data.expected * 100) : 0
  const trendData = trend ?? []
  const activeTrend = trendData.find((item) => item.date === activeTrendDate) ?? trendData.at(-1)
  const chart = { width: 640, height: 176, left: 30, right: 18, top: 18, bottom: 30 }
  const chartWidth = chart.width - chart.left - chart.right, chartHeight = chart.height - chart.top - chart.bottom
  const point = (item: SubmissionTrendDay, index: number) => ({ x: chart.left + (trendData.length < 2 ? chartWidth / 2 : index / (trendData.length - 1) * chartWidth), y: chart.top + (1 - (item.expected ? item.submitted / item.expected : 0)) * chartHeight })
  const path = trendData.map((item, index) => { const value = point(item, index); return `${index ? 'L' : 'M'}${value.x},${value.y}` }).join(' ')
  return <>
    <div className="report-dashboard__cards">{cards.map(([label, value, employees]) => <button key={label} onClick={() => label === '当日应提交' ? setExpectedOpen(true) : setSelected({ title: `${label}人员`, employees })}><span>{label}</span><strong>{value}</strong><small>{label === '当日应提交' ? '查看名单与人数说明' : '点击查看人员'}</small></button>)}</div>
    <div className="report-dashboard__actions"><button onClick={() => void exportMissing(data)}><Download size={15} />导出未提交名单</button><button onClick={() => void exportDelayed(data)}><Download size={15} />导出延后提交名单</button></div>
    <section className="report-dashboard__charts"><article><header><span>当日提交完成率</span><strong>{rate}%</strong></header><div className="report-dashboard__progress"><i style={{ width: `${rate}%` }} /></div><dl><div><dt>已提交</dt><dd>{data.submitted}</dd></div><div><dt>未提交</dt><dd>{data.missing}</dd></div><div><dt>延后提交</dt><dd>{data.delayed}</dd></div></dl></article><article className="report-dashboard__trend-card"><header><span>近 14 天提交趋势</span><small>悬停查看当天明细</small></header><svg viewBox={`0 0 ${chart.width} ${chart.height}`} role="img" aria-label="近14天日报提交完成率趋势" onMouseLeave={() => setActiveTrendDate(null)}><line x1={chart.left} y1={chart.top} x2={chart.width - chart.right} y2={chart.top} /><line x1={chart.left} y1={chart.top + chartHeight / 2} x2={chart.width - chart.right} y2={chart.top + chartHeight / 2} /><line x1={chart.left} y1={chart.top + chartHeight} x2={chart.width - chart.right} y2={chart.top + chartHeight} /><path d={path} />{trendData.map((item, index) => { const value = point(item, index), active = activeTrend?.date === item.date; return <g key={item.date} className={active ? 'active' : ''} onMouseEnter={() => setActiveTrendDate(item.date)}><circle cx={value.x} cy={value.y} r={active ? 6 : 4} /><text x={value.x} y={chart.height - 8}>{item.date.slice(5)}</text></g> })}</svg>{activeTrend && <div className="report-dashboard__trend-detail"><strong>{activeTrend.date}</strong><span>提交 {activeTrend.submitted}/{activeTrend.expected} · 未提交 {activeTrend.missing} · 延后 {activeTrend.delayed}</span></div>}</article></section>
    <section className="report-card"><header><h2>部门完成情况</h2><span>{data.date}</span></header><div className="report-departments">{data.departments.map((department) => { const departmentRate = department.expected ? Math.round(department.submitted / department.expected * 100) : 0; return <button key={department.name} onClick={() => setSelected({ title: `${department.name}提交情况`, employees: data.employees.filter((item) => item.department === department.name) })}><div><strong>{department.name}</strong><span>{department.submitted}/{department.expected} 人</span></div><i><b style={{ width: `${departmentRate}%` }} /></i><small>完成 {departmentRate}%・未提交 {department.missing}・延后 {department.delayed}</small></button> })}</div></section>{selected && <EmployeeDialog {...selected} onClose={() => setSelected(null)} />}{expectedOpen && <ExpectedEmployeesDialog employees={data.employees} excluded={data.excluded} onClose={() => setExpectedOpen(false)} />}</>
}
