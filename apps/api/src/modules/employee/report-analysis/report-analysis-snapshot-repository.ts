...
export class ReportAnalysisSnapshotRepository {
  ...
  async save(startDate: string, endDate: string, snapshot: Omit<ReportAnalysisSnapshot, 'generatedAt'>, accountId: string): Promise<ReportAnalysisSnapshot> {
    const result = await this.pool.query<{ created_at: string | Date }>('INSERT INTO daily_report_analysis_snapshots (start_date,end_date,content,report_references,report_count,created_by_account_id) VALUES ($1::date,$2::date,$3,$4::jsonb,$5,$6) RETURNING created_at', [startDate, endDate, snapshot.content, JSON.stringify(snapshot.references), snapshot.reportCount, accountId])
